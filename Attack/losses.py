...
class CombinedLoss(nn.Module):
    ...
    def forward(self, input, target, weight=1, device="cuda"):
        ...
        if input.size(1)==1: # Binary segmentation

            input = input.to(device)
            target = target.to(device).float()  # Ensure target is of type float
            #print(f"input size is :{input.size(1)}")
            criterion = nn.BCEWithLogitsLoss()
            loss = criterion(input, target)

            #loss=torch.mean(self.dice_loss(input_soft,target)) # it is not combined, it is just dice loss
        else:
